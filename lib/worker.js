  ...
  _createClass(Job, [{
    ...
    value: function _failJob(job) {
      ...
      return this.client.update({
        ...
        if (err.statusCode === 409) return true;
        _this3.debug('_failJob failed on job ' + job._id, err);
        _this3.emit('fail_error', err);
        return false;
      });
    }
  }, {
    ...
    value: function _performJob(job) {
      ...
      var workerOutput = new Promise(function (resolve, reject) {
        resolve(_this4.workerFn.call(null, job._source.payload));
      ...
      return workerOutput.then(function (output) {
        ...
        return _this4.client.update({
          ...
          _this4.emit('job_error', err);
        });
      }, function (jobErr) {
        // job execution failed
        if (jobErr.type === 'WorkerTimeout') {
          _this4.debug('Timeout on job ' + job._id);
          _this4.emit('job_timeout', jobErr);
          return;
        }

        _this4.debug('Failure occurred on job ' + job._id, jobErr);
        _this4.emit('job_error', jobErr);
        return _this4._failJob(job, jobErr.toString());
      });
    }
  }, {
