...
var Job = function (_events$EventEmitter) {
  ...
  function Job(queue, type, workerFn) {
    ...
    _this.queue = queue;
    _this.client = _this.queue.client;
    _this.jobtype = type;
  ...
  _createClass(Job, [{
    ...
    value: function _performJob(job) {
      ...
      var workerOutput = new Promise(function (resolve, reject) {
        resolve(_this2.workerFn.call(null, job._source.payload));
      ...
      return workerOutput.then(function (output) {
        ...
        _this2.debug('Completed job ' + job._id);

        var completedTime = (0, _moment2.default)().toISOString();
        var docOutput = _this2._formatOutput(output);
        ...
        if (jobErr.type === 'WorkerTimeout') {
          _this2.debug('Timeout on job ' + job._id);
          _this2.emit('job_timeout', jobErr);
          return;
        }

        _this2.debug('Failure occurred on job ' + job._id, jobErr);
        _this2.emit('job_error', jobErr);
        return _this2._failJob(job, jobErr.toString());
      });
    }
  }, {
    ...
    value: function _startJobPolling() {
      ...
      this._checker = setInterval(function () {
        _this3._getPendingJobs().then(function (jobs) {
          return _this3._claimPendingJobs(jobs);
        });
      }, this.checkInterval);
    }
  }, {
    ...
    value: function _claimPendingJobs(jobs) {
      ...
      return jobs.reduce(function (chain, job) {
        return chain.then(function (claimedJob) {
          ...
          return _this4._claimJob(job).then(function (claimResult) {
        ...
        _this4.debug('Failed to claim outstanding jobs', err);
        _this4.emit('error', err);
        _this4.queue.emit('worker_error', {
          id: _this4.id,
          type: _this4.type,
          err: err
        });
        throw err;
      }).then(function (claimedJob) {
        if (!claimedJob) {
          _this4.debug('All ' + jobs.length + ' jobs already claimed');
          return;
        }
        _this4.debug('Claimed job ' + claimedJob._id);
        return _this4._performJob(claimedJob);
      }).then(function () {
        return _this4._startJobPolling();
      }).catch(function (err) {
        _this4.debug('Error claiming jobs', err);
        _this4.emit('error', err);
        _this4._startJobPolling();
      });
    }
  }, {
    key: '_getPendingJobs',
    value: function _getPendingJobs() {
      ...
      return this.client.search({
        ...
        _this5.debug(jobs.length + ' outstanding jobs returned');
        return jobs;
      }).catch(function (err) {
        ...
        _this5.queue.emit('worker_error', {
          id: _this5.id,
          type: _this5.type,
          err: err
        });
